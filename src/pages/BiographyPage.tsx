  ...
  return (
    <div className="page active" id="page-biography">
      <nav className="nav">
        ...
        <button type="button" className="nav-back nav-control-btn" onClick={() => scrollToSection('hero')}>
          ← Retour à l'accueil
        </button>
      ...
      <section className="bio-expertise sec">
        ...
        <div className="bio-expertise-wrap">
          {bioExpertiseItems.map((item) =>
            item.href ? (
              <a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer" className={item.className}>
                {item.icon}
                {item.label}
              </a>
            ) : (
