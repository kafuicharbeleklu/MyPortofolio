...
const Projects: React.FC<ProjectsProps> = ({
  ...
  const v = t[lang];
  const mobileProjectsCarousel = useMobileScrollDots(recentProjects.length);
  ...
  return (
    <section className="sec" id="projets">
            ...
            <div className="pf-foot-row">
              <div className="tags">
                {featuredProject.tags.map((tag) => (
                  <span key={`${featuredProject.id}-${tag.label}`} className={`tag ${tag.className}`}>
                    {tag.label}
                  </span>
                ))}
              </div>
      ...
      <div
        className="pj-grid projects-main-grid"
        ref={mobileProjectsCarousel.containerRef}
      >
      ...
      {recentProjects.length > 1 ? (
        <div
          ...
          aria-label={lang === 'FR' ? 'Position dans les réalisations' : 'Projects position'}
        >
          {recentProjects.map((project, index) => (
            <button
              key={`project-dot-${project.id}`}
              type="button"
              className={`carousel-dot ${mobileProjectsCarousel.activeIndex === index ? 'active' : ''}`}
              onClick={() => mobileProjectsCarousel.scrollToIndex(index)}
              aria-label={
                lang === 'FR'
                  ? `Voir la réalisation ${index + 1}`
                  : `View project ${index + 1}`
              }
              aria-current={mobileProjectsCarousel.activeIndex === index ? 'true' : undefined}
            />
          ))}
        </div>
      ) : null}
